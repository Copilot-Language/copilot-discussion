...
\newcommand{\reporttitle}{Verification and traceability of an EDSL generated code.}     % Titre
\newcommand{\reportauthor}{Georges-Axel \textsc{Jaloyan}} % Auteur
\newcommand{\reportsubject}{Undergraduate internship report\\[0.5cm]8 June 2015 - 14 August 2015} % Sujet
...
\end{minted}
	...
	There are two primary stream operators in Copilot : (++) which prepends a finite list to a Stream, and drop, which drops a finite number of elements at the beginning of a Stream. Given that a monitor cannot predict the future, it is impossible to drop elements from a Stream that has no elements prepended using (++).
...
\end{minted}
	...
	*/
	...
	Copilot is first reified using standard observation techniques\footnote{These techniques can be found in \cite{Copilot06}}. In its reified form, it can either be interpreted, printed or compiled, using two different back-ends, the first one using Atom, the second one SBV. Both compilers generate C-code, which can be afterwards compared to each other, to ensure that both are semantically equivalent, using model checking techniques. The Figure~\ref{fig:Copilot backend} shows a global view of the different tools used in Copilot.
	\begin{figure}[ht!]
		\centering
		\footnotesize
...
\end{minted}
	...
	\begin{figure}[ht!]
		...
		(PP) edge [->,blue, anchor=east, text width=2cm] node {ACSL generation} (ACSL)
		...
		%% (SBV) edge [bend left] node {1,0,R} ();
		\end{tikzpicture}
		\caption{The new Copilot toolchain. The red arrows are the one to implement in the future. The blue ones are the ones implemented by the author.}
	...
	\begin{figure}[!htb]
		\centering
		\includegraphics[width=100mm]{images/CAMsep.jpg}
		\caption{Visualization of Horizontal Criterion for Conflict Avoidance from \cite{NMH14ATIO}.}
		\label{fig:CAMsep}
	\end{figure}
...
\begin{figure}[ht!]
	...
	(GUI) edge node {} (AL);
	\end{tikzpicture}
	\label{fig:monstructure} \caption{The overall structure of the monitor}
\end{figure}
...
\end{minipage}
	...
	\begin{minipage}{\textwidth}
		...
		\begin{multicols}{2}
			...
			{\normalsize 	Simplified typing rules for Copilot language.}
