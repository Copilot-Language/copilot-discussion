...
that signals valid packet candidates can be given by 2 more definitions
...
packet or that spans multiple actually sent packets\todo{may quote http://www.usenix.org/events/woot11/tech/final\_files/Goodspeed.pdf ?}).
...
to a system that otherwise lacks that capability.  Not only does Copilot allow
one to synthesize fault-tolerance from high-level specifications, but it
enforces modularity in the interface between the application and
fault-tolerance.  
