...
{\tt w} is defined as follows, given our previous definition of {\tt x}:
%
\begin{code}
w = [5,6,7] ++ x
\end{code}
...
Figure~\ref{fig:explicit_sharing} are semantically identical.  However, by
sharing expression definitions, the size of a Copilot expression can be
dramatically reduced.  Printing the expressions results in something like the
following:
%
\begin{code}
...
%
\noindent
Note that in {\tt s1}, because the sharing is at the Haskell level, the
...
to  assist in assurance of the monitors. The Copilot toolchain is depicted in
...
used to prototype Copilot programs as well as for verification and
validation purposes. Reified programs can be printed using the custom
pretty-printing tool.  Currently, two backends supporting Atom and SBV  are
...
addition, a custom Quickcheck engine and a  test harness are provided
along with support for model checkers to  aide in verifying the
generated C monitors.  We will demonstrate these tools and their usage
in the remainder of this section.  A more thorough discussion about the
