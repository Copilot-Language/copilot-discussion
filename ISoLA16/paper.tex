...
protections when the a system cannot be assured by conventional means,
but only if the RV itself can be trusted.  In this paper,  we proffer a
number of challenges to realizing high-assurance RV and illustrate how
we have addressed them in our own investigations.  We argue that the
high-assurance RV provides a rich target for static analysis tools and
proof engines in hope of fostering closer collaboration between  among
the communities. 
\end{abstract}
\input{intro}
\input{copilot} 
