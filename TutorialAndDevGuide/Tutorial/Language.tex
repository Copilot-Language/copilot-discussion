...
\textbf{Here we discuss how external variables relate to sampling. }
As discussed in section 1.4, \emph{sampling} is a way of monitoring based on 
sampling state-variables, while assuming synchrony between the monitor and 
the observed software. 
...
Please also note that let bindings are not implemented in ACSL, hence it is
impossible to run frama-c on a Copilot program that uses local bindings.

\subsection{Magic labels}
\label{sec:magiclabels}

It is possible to add labels in a copilot program. Labels are essentially
working the same way as the \texttt{trace} function from \texttt{Debug.Trace},
but instead of outputting on stdout, they print their string in the C source
file.
...
in labels, which would modify the code generation. Up to now, only one has been
implemented, which does a beta expansion where it is put. To invoke the
...
where \texttt{expr} is any stream. This would result in splitting the AST
generated into two smaller ASTs, hence this is equivalent to the transformation
$ e \rightarrow_{\beta} (\lambda x . x) e $, where $(\lambda x . x)$ is called
as an external function (given that there is no polymorphism in Copilot, one
identity function per type has been written, which explains why the expression
labeled has to be of the \texttt{Typed} typeclass). This fundamentally does not
...
verified with \texttt{frama-c}.

Here is an example of the splitting process in Figure~\ref{fig:splitting label}
