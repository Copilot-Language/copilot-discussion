...
There also exists an {\tt unsafeCast} operator which allows casting from any type to any other (except from floating point numbers to integer types) :
...
respectively. Please see the grammar at \ref{sec:BNF} in order to have the list of all sampling functions.
...
Please also note that let bindings are not implemented in ACSL, hence it is impossible to run frama-c on a Copilot program that uses local bindings.
...
Here is an example of the splitting process in the Figure~\ref{fig:splitting label} for the following simple Copilot code:
...
\begin{figure}[!htb]
	\includegraphics[width=46mm]{images/label/main.pdf}
	\centering
	\includegraphics[width=42mm]{images/label/splitted.pdf}
	\includegraphics[width=42mm]{images/label/splitted2.pdf}
