...
\jonathan{Is model checking the \textit{most widely used} technique ? What about abstract interpretation ? Of course, modern techniques often consists in a mix of both of them but maybe your sentence is a bit radical. }
...
the failure is produced as a {\em counterexample.} \jonathantxt{The main difficulty with this technique is the huge size of usual search spaces. To tackle this, we use logical formula as a concise description for sets of states and the transition relation in the graph of states is decided by SMT solvers.} Our work applies
...
\jonathan{I do not know the definition of \textit{bounded} model checking and it doesn't seem obvious what the term \textit{bounded} relates to. Indeed, in most of our examples, the search space is not bounded except if you consider integers to be bounded, which is usually not the case in most SMT theories. Anyway, even if it were, the search space is so large that it's a reasonable approximation of what infinity is :). Therefore, we should add a sentence to make this definition clearer.     }

Bounded model checking searches for
counterexamples of length bounded by some user provided integer $k.$
\jonathan{Are you sure of this definition ? In fact, counterexamples are necessary bounded as a consequence of the fact that we restrain ourselves to \textit{safety} properties. Anyway, it could be a good idea to move here the definition of a safety property.}
...
done automatically.  \jonathan{I don't think it is the main problem. In fact, if the inductive step holds, the SMT solver will \textit{always} figure it out if the theories we are using are decidable, although it could take too long. The main problem is that most interesting properties can't be proven by induction at all : most of them are not inductive.} Sheeran~\etal~\cite{Sheeran00, EenS03} introduced
